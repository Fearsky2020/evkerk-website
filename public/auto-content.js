(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .auto-sermon-title-link{color:inherit;text-decoration:none}.sermon-title-tail{white-space:nowrap}.auto-sermon-title-link:hover{text-decoration:underline;text-decoration-color:var(--brand-blue);text-underline-offset:5px}.auto-sermon-title-link:focus-visible{outline:3px solid rgba(17,171,227,.3);outline-offset:4px;border-radius:4px}
  ...
  function sermonTitleHtml(value) {
    const raw = String(value || '');
    const match = raw.match(/^(.*?)(（[^（）]{1,12}）)$/);
    if (!match) return esc(raw);
    return esc(match[1]) + '<span class="sermon-title-tail">' + esc(match[2]) + '</span>';
  }
