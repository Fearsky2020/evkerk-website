(() => {
  ...
  function render(){if(!sermons.length){grid.innerHTML=`<div class="archive-empty">${isNl()?'Er zijn nog geen preken gepubliceerd.':'目前还没有发布讲道。'}</div>`;return}grid.innerHTML=sermons.map(item=>{const title=pick(item.title_zh,item.title_nl)||(isNl()?'Preek':'讲道');const summary=pick(item.summary_zh,item.summary_nl);return `<article class="sermon-card"><time>${esc(dateLabel(item.sermon_date))}</time><h2>${esc(title)}</h2><div class="sermon-meta">${item.speaker?`<span>${esc(item.speaker)}</span>`:''}${item.scripture?`<span>· ${esc(item.scripture)}</span>`:''}</div><p>${esc(summary||(isNl()?'Open de preek voor meer informatie.':'打开讲道查看详细内容。'))}</p><div class="sermon-links"><a href="/sermon?id=${encodeURIComponent(item.id)}">${isNl()?'Bekijk preek':'查看讲道'}</a>${item.youtube_url?`<a href="${esc(item.youtube_url)}" target="_blank" rel="noopener">YouTube</a>`:''}${item.audio_url?`<a href="${esc(item.audio_url)}" target="_blank" rel="noopener">${isNl()?'Audio':'音频'}</a>`:''}</div></article>`}).join('')}
