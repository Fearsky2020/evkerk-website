...
function collectProgress(){
  ...
  return { product: 'evkerk-learn-nl', version: BACKUP_VERSION, exportedAt: new Date().toISOString(), data };
}
...
function downloadBackup(){
  ...
  a.href = url; a.download = `nederlands-leren-backup-${day}.json`;
...
function injectPortable(){
  ...
  section.className = 'portable-section'; section.id = 'portableTools';
  section.innerHTML = `<div class="shell"><article class="portable-card"><div class="portable-head"><div><p class="eyebrow">ZONDER ACCOUNT · 0€</p><h2>不注册，也别丢学习进度。</h2></div><p class="zh-help">学习记录留在本机，需要换设备时导出一个备份文件，再在新设备恢复。</p></div><div class="portable-grid"><div class="portable-box"><h3>📦 学习进度备份</h3><p class="zh-help">包含等级、生词本、FSRS 复习、听力/测验成绩、本周任务和页面偏好；不包含麦克风录音。</p><div class="portable-actions"><button class="btn primary" id="exportProgress" type="button">导出备份</button><label class="btn secondary portable-import">恢复备份<input id="importProgress" type="file" accept="application/json,.json"></label></div><div class="portable-status" id="backupStatus">数据仍只在这台设备里。</div></div><div class="portable-box"><h3>📱 像 App 一样打开</h3><p class="zh-help">核心课程会缓存到浏览器。安装到桌面以后入口更顺手；第一次加载智能组件和 Easy Dutch 仍需要联网。</p><div class="portable-actions"><button class="btn primary" id="installLearnNl" type="button" hidden>安装到设备</button></div><div class="portable-status"><span class="portable-dot" id="networkDot"></span><span id="networkText"></span></div><div class="install-tip zh-help" id="installTip">iPhone / iPad：Safari 分享按钮 →“添加到主屏幕”。Android / 桌面 Chrome：可使用浏览器的“安装应用”。</div></div></div></article></div>`;
  future.parentNode.insertBefore(section, future);
}

function initBackup(){
  const exportButton = q('exportProgress'), importInput = q('importProgress'), status = q('backupStatus');
  exportButton?.addEventListener('click', () => { downloadBackup(); if (status) status.textContent = '备份已生成。把这个 JSON 留好，换设备时可以恢复。'; });
  importInput?.addEventListener('change', async () => {
    const file = importInput.files?.[0]; if (!file) return;
    try { const count = await restoreBackup(file); if (status) status.textContent = `已恢复 ${count} 项学习数据。页面将刷新。`; setTimeout(() => location.reload(), 700); }
    catch (error) { if (status) status.textContent = `恢复失败：${error.message}`; }
    finally { importInput.value = ''; }
  });
}

function initNetwork(){
  const dot = q('networkDot'), text = q('networkText');
  const render = () => { const online = navigator.onLine; dot?.classList.toggle('online', online); dot?.classList.toggle('offline', !online); if (text) text.textContent = online ? '当前在线 · 会更新缓存' : '当前离线 · 使用已缓存的核心课程'; };
  addEventListener('online', render); addEventListener('offline', render); render();
}

function initInstall(){
  const button = q('installLearnNl'); let promptEvent = null;
  addEventListener('beforeinstallprompt', event => { event.preventDefault(); promptEvent = event; if (button) button.hidden = false; });
