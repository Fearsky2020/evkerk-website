(() => {
  ...
  const SETTINGS_KEY = 'taalvia-lock-settings-v1';
  const STATE_KEY = 'taalvia-lock-learning-v1';
  const TODAY_KEY = 'taalvia-lock-today-v1';

  // Curated learning layer on top of the canonical 5000-word frequency corpus.
  // `w` is the exact token used for frequency ranking; `d` is learner-facing display.
  const WORDS = [
  ...
  const els = {};
  let settings = loadJson(SETTINGS_KEY, {level:'start',showChinese:true,showExample:true,lockNotify:false});
  ...
  function currentPool() {
    ...
    if (rankMap.size) {
      ...
      if (inCorpus.length >= WORDS_PER_DAY) pool = inCorpus;
    }
    return pool.slice().sort((a,b) => (rankMap.get(a.w)||999999) - (rankMap.get(b.w)||999999));
  }
  function chooseToday() {
    const key = dateKey();
    const snapshot = loadJson(TODAY_KEY, {});
    const signature = `${VERSION}|${key}|${settings.level}`;
    if (snapshot.signature === signature && Array.isArray(snapshot.words) && snapshot.words.length === WORDS_PER_DAY) {
      const restored = snapshot.words.map(token => WORDS.find(item => item.w === token)).filter(Boolean);
      if (restored.length === WORDS_PER_DAY) return restored;
    }

    const pool = currentPool();
    const known = new Set(learning.known);
    const hard = new Set(learning.hard);
    const preferredHard = pool.filter(item => hard.has(item.w) && !known.has(item.w));
    const fresh = pool.filter(item => !known.has(item.w));
    const candidate = fresh.length >= WORDS_PER_DAY ? fresh : pool;
    const random = rng(hash(signature));
    const picked = [];

    if (preferredHard.length) {
      picked.push(preferredHard[Math.floor(random() * preferredHard.length)]);
    }
    while (picked.length < WORDS_PER_DAY && picked.length < candidate.length) {
      const word = candidate[Math.floor(random() * candidate.length)];
      if (!picked.some(item => item.w === word.w)) picked.push(word);
    }
    while (picked.length < WORDS_PER_DAY) {
      const word = pool[Math.floor(random() * pool.length)];
      if (!picked.some(item => item.w === word.w)) picked.push(word);
    }

    saveJson(TODAY_KEY, {signature,words:picked.map(item => item.w)});
    return picked;
  }
  ...
  function bindElements() {
    ['previewTime','wordMeta','wordDutch','wordType','wordZh','wordExample','exampleNl','exampleZh','prevWord','nextWord','wordDots',
      'todayGrid','levelSelect','showChinese','showExample','lockNotify','settingsStatus','installButton','notificationButton',
  ...
  function renderCurrent() {
    ...
    els.wordMeta.textContent = `WOORD ${current+1} / 3${rank ? ` · TOP ${rank}` : ''}`;
  ...
  function renderTodayGrid() {
    ...
    today.forEach((word,index) => {
      ...
      article.innerHTML = `<span class="slot">WOORD ${index+1} / 3</span><h3></h3><p class="meaning"></p><p class="tiny-example"></p><div class="card-actions"><button type="button" data-action="known">✓ 认识了</button><button type="button" data-action="hard">↻ 不熟，再来</button></div>`;
      ...
      const knownButton = article.querySelector('[data-action=known]');
      const hardButton = article.querySelector('[data-action=hard]');
      knownButton.classList.toggle('active', learning.known.includes(word.w));
      hardButton.classList.toggle('active', learning.hard.includes(word.w));
  ...
  function renderSettings() {
    ...
    els.settingsStatus.textContent = `已保存到本机 · 今日固定 ${WORDS_PER_DAY} 词 · ${corpusState}`;
  }
  function renderAll() {
    renderClock();
    renderDots();
  ...
  function markKnown(token) {
    ...
    learning = {known:[...known],hard:[...hard]};
    saveJson(STATE_KEY,learning);
  ...
  function saveSettings(regenerate = false) {
    settings = {
      ...
      showChinese: els.showChinese.checked,
      showExample: els.showExample.checked,
      lockNotify: els.lockNotify.checked
    };
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      console.warn('[TAALVIA Lock] service worker registration failed.',error);
      return null;
    }
  }
  ...
  async function showNotification(index = current) {
    ...
    target?.postMessage({
      type:'TAALVIA_LOCK_SHOW',
      card:{
        word:word.w,display:word.d,zh:word.z,example:word.e,
        slot:index+1,showChinese:settings.showChinese,showExample:settings.showExample
      }
    });
    return true;
  }
  async function enableNotifications() {
    if (!('Notification' in window)) {
      els.settingsStatus.textContent = '这台浏览器不支持网页通知。';
      els.lockNotify.checked = false;
      saveSettings();
      return;
    }
    ...
    if (permission !== 'granted') {
      els.settingsStatus.textContent = '系统没有授予通知权限。';
      els.lockNotify.checked = false;
      saveSettings();
      return;
    }
    els.lockNotify.checked = true;
    saveSettings();
  ...
  function cardBlob(word,index) {
    return new Promise(resolve => {
      ...
      gradient.addColorStop(0,'#102c56'); gradient.addColorStop(.52,'#146779'); gradient.addColorStop(1,'#0c8b76');
      ctx.fillStyle=gradient; ctx.fillRect(0,0,1290,2796);
      ...
      ctx.fillStyle='#d1fae5'; ctx.font='800 34px system-ui,sans-serif'; ctx.fillText(`TAALVIA LOCK  ·  ${index+1} / 3`,100,300);
      ctx.fillStyle='#ffffff'; ctx.font='800 118px system-ui,sans-serif';
      const wordLines=wrapCanvasText(ctx,word.d,1090);
      wordLines.slice(0,2).forEach((line,i)=>ctx.fillText(line,100,980+i*135));
      let y=980+(wordLines.length*135)+55;
      ctx.fillStyle='#d8f3f0'; ctx.font='600 40px system-ui,sans-serif'; ctx.fillText(word.t,100,y);
      if(settings.showChinese){
        y+=125; ctx.fillStyle='#ffffff'; ctx.font='700 62px system-ui,sans-serif';
        wrapCanvasText(ctx,word.z,1090).slice(0,2).forEach((line,i)=>ctx.fillText(line,100,y+i*78));
        y+=95;
      }
      if(settings.showExample){
        y+=90; ctx.strokeStyle='rgba(255,255,255,.26)'; ctx.lineWidth=2; ctx.beginPath();ctx.moveTo(100,y);ctx.lineTo(1190,y);ctx.stroke();
        y+=105; ctx.fillStyle='#ffffff'; ctx.font='650 45px system-ui,sans-serif';
        wrapCanvasText(ctx,word.e,1090).slice(0,3).forEach((line,i)=>ctx.fillText(line,100,y+i*62));
        if(settings.showChinese){
          y+=190; ctx.fillStyle='#d6e8e9';ctx.font='500 35px system-ui,sans-serif';
          wrapCanvasText(ctx,word.c,1090).slice(0,3).forEach((line,i)=>ctx.fillText(line,100,y+i*52));
        }
      }
      ctx.fillStyle='rgba(255,255,255,.75)';ctx.font='700 28px system-ui,sans-serif';ctx.fillText('taalvia.nl  ·  Unlock your Dutch',100,2605);
      ctx.fillStyle='rgba(255,255,255,.55)';ctx.font='500 24px system-ui,sans-serif';ctx.fillText(dateKey(),100,2660);
  ...
  async function exportToday() {
    ...
    try {
      ...
      for(let i=0;i<today.length;i++){
        const blob = await cardBlob(today[i],i);
        files.push(new File([blob],`taalvia-lock-${dateKey()}-${i+1}.png`,{type:'image/png'}));
      }
      if (navigator.share && navigator.canShare?.({files})) {
        await navigator.share({files,title:'TAALVIA 今日锁屏单词',text:'今天的 3 张荷兰语锁屏单词卡'});
      } else {
        files.forEach((file,index)=>{
          const link=document.createElement('a');
          link.href=URL.createObjectURL(file);link.download=file.name;
          document.body.append(link);link.click();link.remove();
          setTimeout(()=>URL.revokeObjectURL(link.href),3000+index*500);
        });
      }
    } catch (error) {
      if (error?.name !== 'AbortError') console.error('[TAALVIA Lock] export failed.',error);
    } finally {
      els.exportCards.disabled=false;els.exportCards.textContent='导出今日 3 张锁屏图';
    }
  }

  function bindEvents() {
    els.prevWord.addEventListener('click',()=>{current=(current+2)%3;renderCurrent();});
    els.nextWord.addEventListener('click',()=>{current=(current+1)%3;renderCurrent();});
    ...
    els.lockNotify.addEventListener('change',async()=>{
      if(els.lockNotify.checked) await enableNotifications();
      else saveSettings();
    });
    ...
    els.testNotification.addEventListener('click',async()=>{
      ...
      current=(current+1)%3;renderCurrent();await showNotification(current);
    });
    ...
    els.installButton.addEventListener('click',async()=>{
      ...
      deferredInstallPrompt=null;els.installButton.hidden=true;
    });
    window.addEventListener('beforeinstallprompt',event=>{
      event.preventDefault();deferredInstallPrompt=event;els.installButton.hidden=false;
    });
    window.addEventListener('appinstalled',()=>{deferredInstallPrompt=null;els.installButton.hidden=true;});
    document.addEventListener('visibilitychange',async()=>{
      if(document.visibilityState==='hidden'){
        hiddenAt=Date.now();
        if(settings.lockNotify && Notification.permission==='granted'){
          current=(current+1)%3;
          await showNotification(current);
        }
  ...
  function applySlotFromUrl() {
    const slot = Number(new URLSearchParams(location.search).get('slot'));
    if (slot>=1 && slot<=3) current=slot-1;
  }
  function deviceCopy() {
    const ua=navigator.userAgent.toLowerCase();
    if(/iphone|ipad|ipod/.test(ua)) {
      els.deviceHint.textContent='iPhone：点“导出今日 3 张锁屏图”，保存到照片后，在 iPhone 锁屏的“照片随机显示”里选中这三张。网页本身不能监听 iOS 解锁事件。';
    } else if(/android/.test(ua)) {
      els.deviceHint.textContent='Android：安装 TAALVIA Lock 并允许通知。页面从前台进入后台/锁屏时，Service Worker 会显示下一张单词通知；浏览器本身没有系统“解锁事件”权限。';
    }
  }
