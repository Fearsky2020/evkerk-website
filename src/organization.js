...
const QUESTION_STATUSES=new Set(['pending','discussed','closed']);
const QUESTION_SOURCES=new Set(['ios','android','website']);
const isoTime=v=>v?String(v).replace(' ','T').replace(/Z$/,'')+'Z':null;
function questionCursor(row){return btoa(row.created_at+'\n'+row.id).replaceAll('+','-').replaceAll('/','_').replaceAll('=','')}
function parseQuestionCursor(value){try{const raw=String(value||'').replaceAll('-','+').replaceAll('_','/'),p=raw+'='.repeat((4-raw.length%4)%4),[createdAt,id]=atob(p).split('\n');return createdAt&&id?{createdAt,id}:null}catch{return null}}
...
async function appSubmission(request,env,clientRequestId,photo=false){
 ...
 const item=await env.DB.prepare("SELECT id,status,submission_status,expected_photo_count,reception_site,faith_status,created_at,updated_at FROM welcome_cases WHERE submitted_by_member_id=? AND client_request_id=?").bind(u.member_id,clientRequestId).first();if(!item)return json({ok:false,error:'App 提交记录不存在'},404);
 if(!photo){const p=await env.DB.prepare("SELECT id,mime_type,filename,size_bytes,created_at FROM welcome_case_photos WHERE case_id=? AND status='active' ORDER BY created_at").bind(item.id).all();return json({ok:true,submission:{client_request_id:clientRequestId,...presentWelcomeCase(item),photos:p.results||[]}})}
 if(!env.MEDIA){await env.DB.prepare("UPDATE welcome_cases SET submission_status='needs_attention',updated_at=datetime('now') WHERE id=?").bind(item.id).run().catch(()=>{});return json({ok:false,error:'照片暂时无法保存，请稍后重试'},503);}const form=await request.formData().catch(()=>null),file=form?.get('image');if(!file||typeof file.arrayBuffer!=='function')return json({ok:false,error:'请选择信息卡照片'},400);
 ...
 const photoId=id('wphoto'),key=`private/welcome-cards/${item.id}/${photoId}.${ext}`,filename=clean(file.name,180)||`welcome-card.${ext}`;try{await env.MEDIA.put(key,bytes,{httpMetadata:{contentType:mime,contentDisposition:'inline'},customMetadata:{caseId:item.id,photoId}})}catch{await env.DB.prepare("UPDATE welcome_cases SET submission_status='needs_attention',updated_at=datetime('now') WHERE id=?").bind(item.id).run().catch(()=>{});return json({ok:false,error:'照片暂时无法保存，请稍后重试'},503)}
 try{await env.DB.prepare("INSERT INTO welcome_case_photos(id,case_id,r2_key,mime_type,filename,size_bytes,uploaded_by_member_id,status) VALUES(?,?,?,?,?,?,?,'active')").bind(photoId,item.id,key,mime,filename,file.size,u.member_id).run()}catch(error){await env.MEDIA.delete(key).catch(()=>{});await env.DB.prepare("UPDATE welcome_cases SET submission_status='needs_attention',updated_at=datetime('now') WHERE id=?").bind(item.id).run().catch(()=>{});return json({ok:false,error:'照片记录保存失败，已撤销上传；请稍后重试'},500)}
 const countRow=await env.DB.prepare("SELECT COUNT(*) count FROM welcome_case_photos WHERE case_id=? AND status='active'").bind(item.id).first();const uploaded=Number(countRow?.count||0),submissionStatus=!item.expected_photo_count||uploaded>=Number(item.expected_photo_count)?'complete':'photo_pending';await env.DB.prepare("UPDATE welcome_cases SET submission_status=?,updated_at=datetime('now') WHERE id=?").bind(submissionStatus,item.id).run();
 await audit(env,null,'welcome.app_photo_upload','welcome_case',item.id,null,{photo_id:photoId},u.member_id);return json({ok:true,submission_status:submissionStatus,uploaded_photo_count:uploaded,expected_photo_count:Number(item.expected_photo_count||0),photo:{id:photoId,mime_type:mime,filename,size_bytes:file.size}},201);
}
async function appQuestions(request,env,url){
 const u=await appIdentity(request,env);if(!u)return json({ok:false,error:'App 登录已失效'},401);
 if(request.method==='GET'){
  const status=clean(url.searchParams.get('status'),20);if(status&&!QUESTION_STATUSES.has(status))return json({ok:false,error:'问题状态不正确'},400);
  const limit=Math.min(100,Math.max(1,Number(url.searchParams.get('limit'))||30)),cursorRaw=clean(url.searchParams.get('cursor'),500),cursor=cursorRaw?parseQuestionCursor(cursorRaw):null;if(cursorRaw&&!cursor)return json({ok:false,error:'cursor 格式不正确'},400);
  const where=['q.member_id=?'],args=[u.member_id];if(status){where.push('q.status=?');args.push(status)}if(cursor){where.push('(q.created_at<? OR (q.created_at=? AND q.id<?))');args.push(cursor.createdAt,cursor.createdAt,cursor.id)}
  const r=await env.DB.prepare(`SELECT q.id,q.book,q.chapter,q.verse_start,q.verse_end,q.reference,q.scripture_text,q.question,q.status,q.source,q.created_at,q.updated_at,q.discussed_at,q.closed_at,g.group_number,g.name group_name
   FROM group_questions q JOIN church_groups g ON g.id=q.group_id WHERE ${where.join(' AND ')} ORDER BY q.created_at DESC,q.id DESC LIMIT ?`).bind(...args,limit+1).all();
  const rows=r.results||[],more=rows.length>limit,items=rows.slice(0,limit).map(q=>({...q,created_at:isoTime(q.created_at),updated_at:isoTime(q.updated_at),discussed_at:isoTime(q.discussed_at),closed_at:isoTime(q.closed_at),group:{number:q.group_number,name:q.group_name},group_number:undefined,group_name:undefined}));
  return json({ok:true,questions:items,next_cursor:more?questionCursor(rows[limit-1]):null});
 }
 if(!u.scope_list.includes('my-group:question:submit'))return json({ok:false,error:'当前账号没有提交小组问题的权限'},403);
 const group=await env.DB.prepare("SELECT id,group_number,name,cluster_id FROM church_groups WHERE id=? AND cluster_id=? AND status='active' AND is_demo=0").bind(u.group_id,u.cluster_id).first();if(!group)return json({ok:false,error:'当前成员没有绑定有效小组'},404);
 const b=await request.json().catch(()=>({})),book=clean(b.book,80),chapter=Number(b.chapter),verseStart=Number(b.verse_start),verseEnd=b.verse_end==null||b.verse_end===''?null:Number(b.verse_end),reference=clean(b.reference,160),scriptureText=clean(b.scripture_text,10000),question=clean(b.question,4000),clientRequestId=clean(b.client_request_id,120);
 if(!book||!reference||!scriptureText||!question||!Number.isInteger(chapter)||chapter<1||chapter>176||!Number.isInteger(verseStart)||verseStart<1||verseStart>200||(verseEnd!==null&&(!Number.isInteger(verseEnd)||verseEnd<verseStart||verseEnd>200)))return json({ok:false,error:'经文出处、内容或问题格式不正确'},400);
 if(!/^[A-Za-z0-9._:-]{8,120}$/.test(clientRequestId))return json({ok:false,error:'client_request_id 格式不正确'},400);
 const existing=await env.DB.prepare("SELECT id,status,reference,created_at FROM group_questions WHERE member_id=? AND client_request_id=?").bind(u.member_id,clientRequestId).first();if(existing)return json({ok:false,error:'问题已提交，请勿重复提交',existing:true,id:existing.id,status:existing.status,reference:existing.reference,created_at:isoTime(existing.created_at),group:{number:group.group_number,name:group.name}},409);
 const recent=await env.DB.prepare("SELECT COUNT(*) count FROM group_questions WHERE member_id=? AND datetime(created_at)>=datetime('now','-5 minutes')").bind(u.member_id).first();if(Number(recent?.count||0)>=5)return json({ok:false,error:'问题提交过于频繁，请稍后再试'},429);
 const sourceRaw=clean(request.headers.get('x-evkerk-platform')||b.source,20).toLowerCase(),source=QUESTION_SOURCES.has(sourceRaw)?sourceRaw:'website',qid=id('question');
 try{await env.DB.prepare("INSERT INTO group_questions(id,member_id,cluster_id,group_id,book,chapter,verse_start,verse_end,reference,scripture_text,question,client_request_id,status,source) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'pending',?)").bind(qid,u.member_id,u.cluster_id,u.group_id,book,chapter,verseStart,verseEnd,reference,scriptureText,question,clientRequestId,source).run()}catch{const same=await env.DB.prepare("SELECT id,status,reference,created_at FROM group_questions WHERE member_id=? AND client_request_id=?").bind(u.member_id,clientRequestId).first();if(same)return json({ok:false,error:'问题已提交，请勿重复提交',existing:true,id:same.id,status:same.status,reference:same.reference,created_at:isoTime(same.created_at),group:{number:group.group_number,name:group.name}},409);return json({ok:false,error:'小组问题保存失败'},500)}
...
async function groupQuestions(request,env,url,qid=''){
 ...
 if(request.method==='GET'){
  ...
  return json({ok:true,role:x.level,questions:r.results||[]});
 }
 const item=await env.DB.prepare("SELECT q.*,g.cluster_id group_cluster_id FROM group_questions q JOIN church_groups g ON g.id=q.group_id WHERE q.id=?").bind(qid).first();if(!item)return json({ok:false,error:'小组问题不存在'},404);if(!canManageGroupQuestion(x,{cluster_id:item.group_cluster_id,group_id:item.group_id}))return json({ok:false,error:'不得处理其他大组或小组的问题'},403);
 const b=await request.json().catch(()=>({})),nextStatus=clean(b.status,20)||item.status,handlerNote=clean(b.handler_note,4000);if(!QUESTION_STATUSES.has(nextStatus))return json({ok:false,error:'问题状态不正确'},400);
 await env.DB.prepare("UPDATE group_questions SET status=?,handler_note=?,handled_by_user_id=?,discussed_at=CASE WHEN ?='discussed' AND discussed_at IS NULL THEN datetime('now') ELSE discussed_at END,closed_at=CASE WHEN ?='closed' AND closed_at IS NULL THEN datetime('now') ELSE closed_at END,updated_at=datetime('now') WHERE id=?").bind(nextStatus,handlerNote,x.user.id,nextStatus,nextStatus,qid).run();
 const after=await env.DB.prepare("SELECT * FROM group_questions WHERE id=?").bind(qid).first();await audit(env,x,'group_question.update','group_question',qid,item,after);return json({ok:true,question:{id:qid,status:after.status,discussed_at:isoTime(after.discussed_at),closed_at:isoTime(after.closed_at),handler_note:after.handler_note}});
}

async function app(request,env,kind){
 const u=await appIdentity(request,env);if(!u)return json({ok:false,error:'App 登录已失效'},401);
 if(kind==='my-group'){if(!u.scope_list.includes('my-group:read'))return json({ok:false,error:'当前账号没有读取小组资料的权限'},403);const g=await env.DB.prepare("SELECT g.id,g.group_number,g.name group_name,c.name cluster_name,g.cluster_leader_name,g.leader_name,g.meeting_day,g.meeting_time,g.meeting_frequency,g.meeting_address,g.postcode,g.navigation_address,g.contact_phone,g.reception_status,g.weekly_status,g.temporary_change,g.announcement,g.schedule_note FROM church_groups g JOIN church_clusters c ON c.id=g.cluster_id WHERE g.id=? AND g.status='active'").bind(u.group_id).first();if(!g)return json({ok:false,error:'尚未分配有效小组'},404);const ar=await env.DB.prepare("SELECT m.id,m.display_name FROM group_assistants a JOIN church_members m ON m.id=a.member_id WHERE a.group_id=? AND m.status='active' ORDER BY m.display_name").bind(u.group_id).all();const n=await env.DB.prepare("SELECT id,title,body,scope_type,starts_at,ends_at FROM group_notifications WHERE status='active' AND (scope_type='church' OR (scope_type='cluster' AND cluster_id=?) OR (scope_type='group' AND group_id=?)) AND (starts_at IS NULL OR datetime(starts_at)<=datetime('now')) AND (ends_at IS NULL OR datetime(ends_at)>=datetime('now')) ORDER BY created_at DESC").bind(u.cluster_id,u.group_id).all();const qc=await env.DB.prepare("SELECT COUNT(*) count FROM group_questions WHERE group_id=? AND status='pending'").bind(u.group_id).first();return json({ok:true,member:{id:u.member_id,name:u.display_name},group:{...g,assistants:ar.results||[]},notifications:n.results||[],pending_question_count:Number(qc?.count||0)})}
