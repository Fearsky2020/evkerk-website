...
async function appIdentity(request,env){const raw=(request.headers.get('authorization')||'').replace(/^Bearer\s+/i,'');if(!raw)return null;const h=await hash(raw),r=await env.DB.prepare("SELECT t.id token_id,t.scopes,m.id member_id,m.display_name,m.cluster_id,m.group_id FROM member_app_tokens t JOIN church_members m ON m.id=t.member_id WHERE t.token_hash=? AND t.status='active' AND m.status='active' AND (t.access_expires_at IS NULL OR datetime(t.access_expires_at)>datetime('now'))").bind(h).first();if(r){r.scope_list=String(r.scopes||'my-group:read').split(/\\s+/).filter(Boolean);env.DB.prepare("UPDATE member_app_tokens SET last_used_at=datetime('now') WHERE id=?").bind(r.token_id).run().catch(()=>{})}return r}
...
async function app(request,env,kind){
 ...
 if(!u.scope_list.includes('welcome:submit'))return json({ok:false,error:'当前账号没有新人接待提交权限'},403);const b=await request.json().catch(()=>({})),postcode=pc(b.postcode);if(!postcode)return json({ok:false,error:'请填写新人邮编'},400);const wid=id('welcome');await env.DB.prepare("INSERT INTO welcome_cases(id,display_name,contact_note,postcode,age_band,family_status,children_note,occupation_stage,preferred_days,language_note,background_note,status,source,submitted_by_member_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,'new','app',?)").bind(wid,clean(b.display_name,120)||'新朋友',clean(b.contact_note,1000),postcode,clean(b.age_band,80),clean(b.family_status,120),clean(b.children_note,500),clean(b.occupation_stage,120),clean(b.preferred_days,300),clean(b.language_note,300),clean(b.background_note,1000),u.member_id).run();await audit(env,null,'welcome.app_submit','welcome_case',wid,null,{source:'app'},u.member_id);return json({ok:true,id:wid,status:'new'},201);
}
