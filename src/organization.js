...
async function appIdentity(request,env){const raw=(request.headers.get('authorization')||'').replace(/^Bearer\s+/i,'');if(!raw)return null;const h=await hash(raw),r=await env.DB.prepare("SELECT t.id token_id,m.id member_id,m.display_name,m.cluster_id,m.group_id FROM member_app_tokens t JOIN church_members m ON m.id=t.member_id WHERE t.token_hash=? AND t.status='active' AND m.status='active'").bind(h).first();if(r)env.DB.prepare("UPDATE member_app_tokens SET last_used_at=datetime('now') WHERE id=?").bind(r.token_id).run().catch(()=>{});return r}
...
async function app(request,env,kind){
 ...
 if(kind==='my-group'){const g=await env.DB.prepare("SELECT g.id,g.group_number,g.name group_name,c.name cluster_name,g.cluster_leader_name,g.leader_name,g.meeting_day,g.meeting_time,g.meeting_frequency,g.meeting_address,g.postcode,g.navigation_address,g.contact_phone,g.weekly_status,g.temporary_change,g.announcement,g.schedule_note FROM church_groups g JOIN church_clusters c ON c.id=g.cluster_id WHERE g.id=? AND g.status='active'").bind(u.group_id).first();if(!g)return json({ok:false,error:'尚未分配有效小组'},404);const n=await env.DB.prepare("SELECT id,title,body,scope_type,starts_at,ends_at FROM group_notifications WHERE status='active' AND (scope_type='church' OR (scope_type='cluster' AND cluster_id=?) OR (scope_type='group' AND group_id=?)) AND (starts_at IS NULL OR datetime(starts_at)<=datetime('now')) AND (ends_at IS NULL OR datetime(ends_at)>=datetime('now')) ORDER BY created_at DESC").bind(u.cluster_id,u.group_id).all();return json({ok:true,member:{id:u.member_id,name:u.display_name},group:g,notifications:n.results||[]})}
 const b=await request.json().catch(()=>({})),postcode=pc(b.postcode);if(!postcode)return json({ok:false,error:'请填写新人邮编'},400);const wid=id('welcome');await env.DB.prepare("INSERT INTO welcome_cases(id,display_name,contact_note,postcode,age_band,family_status,children_note,occupation_stage,preferred_days,language_note,background_note,status,source,submitted_by_member_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,'new','app',?)").bind(wid,clean(b.display_name,120)||'新朋友',clean(b.contact_note,1000),postcode,clean(b.age_band,80),clean(b.family_status,120),clean(b.children_note,500),clean(b.occupation_stage,120),clean(b.preferred_days,300),clean(b.language_note,300),clean(b.background_note,1000),u.member_id).run();await audit(env,null,'welcome.app_submit','welcome_case',wid,null,{source:'app'},u.member_id);return json({ok:true,id:wid,status:'new'},201);
}
export async function handleOrganizationApi(request,env,url){
 ...
 if(url.pathname==='/api/app/my-group'&&request.method==='GET')return app(request,env,'my-group');if(url.pathname==='/api/app/welcome'&&request.method==='POST')return app(request,env,'welcome');
